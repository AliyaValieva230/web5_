...
class _ReaderFormScreenState extends State<ReaderFormScreen> {
  ...
  Validator _dateValidator() {
    return (v) {
      final t = v?.trim() ?? '';
      if (t.isEmpty) return 'Поле обязательно';
      if (DateTime.tryParse(t) == null) return 'Формат: ГГГГ-ММ-ДД';
      return null;
    };
  }
  ...
  Future<void> _submit() async {
    ...
    final card = LibraryCard(
      ...
      issuedAt: DateTime.parse(_issuedAtController.text.trim()),
      expiresAt: DateTime.parse(_expiresAtController.text.trim()),
      isActive: _isActive,
    );
    final reader = Reader(
      id: widget.id ?? 0,
