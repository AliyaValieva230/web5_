...
class PublisherListNotifier extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    try {
      _result = await _repo.find(
        search: _search, sortField: _sortField, sortAscending: _sortAscending,
        page: _page, size: _size, includeDeleted: _includeDeleted,
      );
  ...
  Future<void> save(Publisher publisher) async {
    if (publisher.id == 0) await _repo.create(publisher);
    else await _repo.update(publisher);
    await _load();
  }
  Future<void> deletePublisher(int id) async {
    final books = await _bookRepo.find(publisherId: id, includeDeleted: true, size: 1);
    if (books.total > 0) {
      throw Exception('Невозможно удалить: ${books.total} книг ссылаются на издательство');
    }
