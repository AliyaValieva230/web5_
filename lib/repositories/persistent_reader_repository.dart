...
final List<Reader> seedReaders = [
  ...
  Reader(id: 2, firstName: 'Мария', lastName: 'Сидорова', email: 'maria@mail.ru'),
];
...
class PersistentReaderRepository implements ReaderRepository {
  ...
  PersistentReaderRepository(this._prefs) { _restore(); }
  ...
  void _restore() {
    ...
    try {
      ...
      _nextId = _readers.isEmpty ? 1 : _readers.map((r) => r.id).reduce((a, b) => a > b ? a : b) + 1;
      _nextCardId = _readers.fold(10, (max, r) => r.card != null && r.card!.id > max ? r.card!.id : max) + 1;
    } catch (e) {
      _readers = List.from(seedReaders);
      _nextId = _readers.map((r) => r.id).reduce((a, b) => a > b ? a : b) + 1;
      _persist();
  ...
  bool isEmailUnique(String email, {int? excludeId}) {
    return !_readers.any((r) => r.email == email && r.id != excludeId && !r.isDeleted);
  }
  ...
  Future<PageResult<Reader>> find({
    ...
    if (search.trim().isNotEmpty) {
      ...
      rows = rows.where((r) =>
          ...
          r.email.toLowerCase().contains(q)).toList();
    }
    rows.sort((a, b) {
      int r;
      switch (sortField) {
        case 'firstName': r = a.firstName.compareTo(b.firstName); break;
        case 'email': r = a.email.compareTo(b.email); break;
        default: r = a.lastName.toLowerCase().compareTo(b.lastName.toLowerCase());
      }
  ...
  Future<Reader?> findById(int id) async {
    try { return _readers.firstWhere((r) => r.id == id); } catch (_) { return null; }
  }

  @override
  Future<Reader> create(Reader reader) async {
    final card = reader.card != null ? reader.card!.copyWith(id: _nextCardId++, readerId: reader.id) : null;
  ...
  Future<Reader> update(Reader reader) async {
    ...
    final updated = reader.copyWith(card: reader.card?.copyWith(readerId: reader.id));
    _readers[i] = updated;
    await _persist();
    return updated;
  }
