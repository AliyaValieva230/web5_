...
final List<Genre> seedGenres = [
  Genre(id: 1, name: 'Роман', description: 'Крупное повествовательное произведение'),
...
class PersistentGenreRepository implements GenreRepository {
  ...
  PersistentGenreRepository(this._prefs) { _restore(); }
  ...
  void _restore() {
    ...
    try {
      ...
      _nextId = _genres.isEmpty ? 1 : _genres.map((g) => g.id).reduce((a, b) => a > b ? a : b) + 1;
    } catch (e) {
      _genres = List.from(seedGenres);
      _nextId = _genres.map((g) => g.id).reduce((a, b) => a > b ? a : b) + 1;
      _persist();
  ...
  Future<PageResult<Genre>> find({
    ...
    if (search.trim().isNotEmpty) {
      ...
      rows = rows.where((g) => g.name.toLowerCase().contains(q) || (g.description?.toLowerCase().contains(q) ?? false)).toList();
    }
  ...
  Future<Genre?> findById(int id) async {
    try { return _genres.firstWhere((g) => g.id == id); } catch (_) { return null; }
  }
