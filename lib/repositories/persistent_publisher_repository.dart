...
class PersistentPublisherRepository implements PublisherRepository {
  ...
  PersistentPublisherRepository(this._prefs) { _restore(); }

  void _restore() {
    final raw = _prefs.getString(_key);
    if (raw == null) {
      _publishers = List.from(seedPublishers);
      _nextId = _publishers.map((p) => p.id).reduce((a, b) => a > b ? a : b) + 1;
      _persist();
      return;
    }
    try {
      ...
      _nextId = _publishers.isEmpty ? 1 : _publishers.map((p) => p.id).reduce((a, b) => a > b ? a : b) + 1;
    } catch (e) {
      _publishers = List.from(seedPublishers);
      _nextId = _publishers.map((p) => p.id).reduce((a, b) => a > b ? a : b) + 1;
      _persist();
  ...
  Future<PageResult<Publisher>> find({
    ...
    var rows = _publishers.where((p) => includeDeleted || !p.isDeleted).toList();
    if (search.trim().isNotEmpty) {
      final q = search.trim().toLowerCase();
      rows = rows.where((p) => p.name.toLowerCase().contains(q) || (p.address?.toLowerCase().contains(q) ?? false)).toList();
    }
  ...
  Future<Publisher?> findById(int id) async {
    try { return _publishers.firstWhere((p) => p.id == id); } catch (_) { return null; }
  }
