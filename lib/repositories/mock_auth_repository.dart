...
class MockAuthRepository implements AuthRepository {
  static final Map<String, ({String password, AppUser user})> _users = {
    'reader': (
      password: 'reader',
      user: const AppUser(
          id: 1, username: 'reader', fullName: 'Иван Читатель', role: Role.reader),
    ),
    'librarian': (
      password: 'librarian',
      user: const AppUser(
          id: 2, username: 'librarian', fullName: 'Пётр Библиотекарь', role: Role.librarian),
    ),
    'admin': (
      password: 'admin',
      user: const AppUser(
          id: 3, username: 'admin', fullName: 'Анна Администратор', role: Role.admin),
    ),
  };
  ...
  Future<AuthResult> register({
    ...
    final user = AppUser(
        id: 100 + _seq++, username: username, fullName: fullName, role: Role.reader);
